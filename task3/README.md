...
## Мое решение
Для решения данной задачи мной был применен алгоритм [сканирующей прямой](https://ru.algorithmica.org/cs/decomposition/scanline/). Моя реализация подходит для решения задачи с k-объектами, где за объекты принимаются все сущности с интервалами (в этой задаче их 3), то есть реализация универсальна. Асимптотика реализации этого алгоритма - O(nlogk), учитывая асимптотику произведения операций над кучей, котора была использована.

Ограничение моей реализации - недопустимость наличия наложения интервалов друг на друга у одного объекта.